"""
File/Program created by Ana Mrcela Espinosa 
...
def get_ipv4_and_ipv6_addresses(domain_name):
    try:
        # Resolve the IPv4 and IPv6 addresses using socket and dns.resolver
        ipv4_addresses = socket.gethostbyname_ex(domain_name)
        ipv6_addresses = socket.getaddrinfo(domain_name, None, socket.AF_INET6)
        return ipv4_addresses, ipv6_addresses
    except (socket.gaierror, dns.exception.DNSException) as e:
        return None, None

def extract_cnames(response):
    cnames = []
...
def is_valid_domain(domain_name):
    try:
        # Check if the domain name is valid by resolving it
...
ipv4_addresses, ipv6_addresses = get_ipv4_and_ipv6_addresses (domain_name)
is_domain_valid = is_valid_domain(domain_name)

if ipv4_addresses:
    print(f"IPv4 Addresses for {domain_name}: {ipv4_addresses[2]}")

if ipv6_addresses:
    print(f"IPv6 Addresses for {domain_name}: {[addr[4][0] for addr in ipv6_addresses]}")

try:
    response = dns.resolver.query(domain_name, dns.rdatatype.CNAME)
    cnames = extract_cnames(response)
    if cnames:
        print(f"CNAMEs for {domain_name}: {', '.join(cnames)}")
    else:
        print(f"No CNAME records found for {domain_name}")
except dns.resolver.NXDOMAIN:
    print(f"No CNAME records found for {domain_name}")

if is_domain_valid:
    print(f"{domain_name} is a valid domain.")
else:
    print(f"{domain_name} is not a valid domain.")
